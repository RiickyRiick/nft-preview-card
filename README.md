...
This project was review for me but I don't overlook any of these simple projects, since they are a good way continue showing me how much I have in improved i.e., how long it takes me to code, am iusing good HTML semantics, do I understand fully the basic fundamentals, etc. 

However, the process in hovering over the image and displaying a white bg color, overlaying another image, and adding a smooth transition is all still fairly new to me. This led me to learned more about how to use a image wrapper in order to execute this hover image overlay. 
...
Approached my HTML and CSS in a more "professional way", by this I mean I focused more on readability and keeping my code organized in sections. I wanted to approach easier projects with a more presentable look. 
...
and gave all the classes a more clearer and concise names. 

____________________________________________________________________________

Another thing I learned was how to overlay a background color, and display an 
overlayed image with the hover pseudo class, and maintain it's positioned. 

I first started by wrapping the images together in order to use .img-wrapper as a position: relative; this will help me add a position: absolute to the .view-icon image, and make it easier to adjust it to the center. Also, another great reason for wrapping these images is that when you hover over the image wrapper it will trigger the .view-icon, instead of only hovering over the .view-icon; more friendly-user (moreon this in the hovering secton fown below):
```html
 <article class="nft-card">
...
I also used percentages for the top, bottom and for the transform, since pixels gave it a fixed size. This caused me problems when shrinking the viewport. When I used percentages in properties like transform: translate(-10%, -10%);, the values are calculated relative to the size of the element itself (in this case, the .view-icon). 
...
```
...so I first added a bg-color: white in order to have the faded effect. I also added an 11px in order for the white background not how from the corners, keepign it close to the same size as the .negt-iamge borders at 10px. I then needed to target the .nft-image and .view-icon, however, as I mentioned before, since I wrapped them both together within the .img-wrapper, I can then used .img-wrapper to tigger the hover effect.
So in other words, the .nft-image and .view-icon image will only trigger its effect when you go over the .img-wrapper, which covers the full width being the parent container. 

### Continued development

I will continue to work with the transition and transform rules. This project was good 
review, and also got to learn more about hover effects, tranisitioning, and transform.
